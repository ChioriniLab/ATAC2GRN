...
READS=1
EXPERIMENT=/data/pranzatellitj/check_HOMER_stitching/experiments_ATAC/$ENCSR
DATA=/data/pranzatellitj/check_HOMER_stitching/titrated_data_ATAC/$ENCSR.fastq
GENOME=/data/ChioriniCompCor/experiments/2017-09-01_Metamachine_Footprinting_Pipelines/genomes/hg19
...
java -Djava.io.tmpdir=. -jar $TRIMMOJAR SE $EXPERIMENT/cat.fastq $EXPERIMENT/trimmed.fastq CROP:20
mv $EXPERIMENT/trimmed.fastq $EXPERIMENT/cat.fastq
...
if [[ $READS == 1 ]]
    ...
    bowtie2 -x $GENOME/Bowtie2Index/genome --local -p 8 -t -q -U $EXPERIMENT/cat.fastq -S $EXPERIMENT/bowtie2.sam
    ...
    # These files are passed to bowtie2 to be aligned to the given genome index.
    bowtie2 -x $GENOME/Bowtie2Index/genome --local -p 8 -t -q -1 $EXPERIMENT/P1.fastq -2 $EXPERIMENT/P2.fastq -S $EXPERIMENT/bowtie2.sam
...
rm $EXPERIMENT/peaks.bedgraph
rm $EXPERIMENT/peaks.merged.bed

cp -R /data/ChioriniCompCor/experiments/2017-06-20_Drews_ATAC_vs_DNase_H2H/ChIP-validation $EXPERIMENT/
