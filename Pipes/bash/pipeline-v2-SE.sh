...
cat $DATA/*.fastq.gz > $EXPERIMENT/1.fastq.gz
java -Djava.io.tmpdir=. -jar $TRIMMOJAR SE $EXPERIMENT/1.fastq.gz $EXPERIMENT/1.fq.gz HEADCROP:20
echo "Paired-end reads were concatenated."
# These files are passed to bowtie2 to be aligned to the given genome index.
bowtie2 -x $GENOME/Bowtie2Index/genome --very-sensitive -p 4 -t -q -U $EXPERIMENT/1.fq.gz -S $EXPERIMENT/bowtie2.sam
